...
class MotherShip : render.Scene
{
	...
	this(string[] grid)
	{
		...
		auto tower = new render.Shape(
			...
			new render.Material(vec3(1, 0, 0), 0.75)
		);
		...
		foreach(y, line; grid)
		{
			foreach(x, c; line)
			{
				...
				switch(c)
				{
					case '|':
					case '-':
					case 'X': add(pos, floor).rot = rot; break;
					case '#': add(pos, wall).rot = rot; break;
					case 'O': add(pos, tower).rot = rot; break;
	...
	override void run()
	{
		...
		void checkturn()
		{
			...
			for(int i = 0; i < steps; i++)
			{
				...
				ship.rot.z += 180.0/steps;
				if(ship.rot.z >= 360) ship.rot.z -= 360;
				root.pos.x += speed;
				speed += d*1;
		...
		for(;;nextframe())
		{
			...
			if(ship.rot.x < ship.rot.z) ship.rot.x += min(8, ship.rot.z - ship.rot.x);
			if(ship.rot.x > ship.rot.z) ship.rot.x -= min(8, ship.rot.x - ship.rot.z);
