...
void play(string mazename)
{
	...
	void add_wall(ulong x, ulong y) {
	}

	void add_empty(ulong x, ulong y) {
		maze.add(x - 0.33, y - 0.33, rect2x2, vec4(0, 0, 0, 1));
	}

	void add_door(ulong x, ulong y) {
		//add_empty(x, y);
		doors.add(x - 0.33, y, doormesh, doormat);
	}

	void add_food(ulong x, ulong y) {
		add_empty(x, y);
		foods.add(x + 0.5, y + 0.5, foodmesh, vec4(0.6, 0.6, 0.2, 1));
		grid[y][x] = ' ';
	}

	void add_ghost(ulong x, ulong y) {
		add_empty(x, y);
	...
	void add_player(ulong x, ulong y) {
		add_empty(x, y);
		auto shape = mobs.add(x + 0.5, y + 0.5, actorbox, new render.Material());
		grid[y][x] = ' ';
